(function($) {
    $.fn.BVSelect = function(parameters) {
        ...
        function SetListBV(options) {
            ...
            $(document).on('click.bv_mainselect', function(event) {
                if ($(event.target).closest('.bv_mainselect').length === 0) {
                    ...
                    selectedIDFocus = 0;
                    $(".bv_ul_inner").css("position", "inherit");
        ...
        function SetBaseBV(options, config) {
            ...
            $("#bv_" + randomID).append('<div id="main_' + randomID + '" class="bv_atual bv_background"></div><ul id="ul_' + randomID + '" class="bv_ul_inner bv_background" style="    position: inherit;"></ul>');

            if (config.searchbox == true) {
                $("#ul_" + randomID).prepend('<li class="nofocus"><div class="innerinput"><input placeholder="Buscar..." class="bv_input" id="input_' + randomID + '" type="text"></div</li>');
            }
        ...
        function FixVerticalViewPort() {
            ...
            if (Math.round(currentElementView + 50) > Math.round(currentWindowView)) {
                ...
                $("#ul_" + randomID).css("height", "200px");
                $("#ul_" + randomID).css("overflow", "auto");
            } else {
                selectedIDFocus = 0;
                $(".bv_ul_inner").css("position", "inherit");
                $(".bv_ul_inner").css("bottom", "");
